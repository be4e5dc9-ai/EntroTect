...
本文描述当前源码中的能力。发布版本和安装包以 GitHub Releases 为准。
...
- **持续工具调用**：执行结果会回传给模型，供其判断下一步。连续的独立读取、搜索最多 8 路并行；文件修改与 Shell 按调用顺序执行。子代理可并行，但与主代理的读取、修改之间会等待完成；中断时保留完整的调用/结果配对。
...
自动压缩及 `/compact` 手动压缩都会在对话当前位置显示轻量状态标记，并支持停止。单次压缩（含重试）最多等待 5 分钟；超时、取消或摘要不完整时保留原上下文并恢复输入。完成标记随历史保存，重开会话仍可看到；内部摘要用于模型续接，不作为普通聊天消息展示。

压缩继承会话的有效思考档位，Ultra 在摘要请求中映射为 Max。摘要目标最多 8,000 tokens；包含思考的生成额度按档位为 16,384–65,536 tokens，因长度截断时最多再扩大一倍重试一次，最高 131,072 tokens，始终受模型已知输出上限及剩余上下文约束。较长的预算是可用上限，并非每次都会用满。
...
上下文可按阈值自动压缩，默认阈值为 70%，可在设置中调整或关闭。压缩会以摘要替换早期消息，保留规划模式与目标状态；会话记录不应视为完整、不可变的原始历史备份。工具输出同时受单次 50KB 和整批约 200KB 的正文预算约束，超限部分保存为文件，并以预览和路径回传。`read` 可分段读取当前会话的输出文件；普通文件超过 256KB 时须指定行范围，每次返回约 40KB 及下一段行号。日志保存失败会明确提示，不把已经完成的操作误报为失败。
