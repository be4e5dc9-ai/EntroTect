...
const inputSchema = z.strictObject({
  ...
  old_string: z.string().min(1, "old_string 不能为空").describe("待替换的原文片段,不能为空且必须在文件中唯一匹配"),
  ...
  async call(rawArgs: unknown, ctx: ToolContext): Promise<string> {
    ...
    return withFileLock(absolute, ctx.abortSignal, async (filePath) => {
      ...
      if (occurrences.length === 0) {
        throw new Error("未找到 old_string。请用 read 确认当前内容(注意缩进与换行符)。");
      }
      if (!args.replace_all && occurrences.length > 1) {
        throw new Error(`old_string 匹配到 ${occurrences.length} 处,不唯一。请扩大上下文使其唯一,或设置 replace_all。`);
      }

      // A replacement string interprets $&, $`, $' and $$; code must be literal.
      const parts = content.split(args.old_string);
      const next = args.replace_all
        ? parts.join(args.new_string)
        : content.replace(args.old_string, () => args.new_string);
      await atomicWriteText(filePath, next, content, ctx.abortSignal);
      recordFileState(ctx, filePath, next);
      const count = args.replace_all ? parts.length - 1 : 1;
