...
- 互不依赖的读取、搜索和子任务可同批调用；运行时会限制并发，写入、编辑与 shell 按调用顺序执行。有数据依赖或需要先检查结果再决定的步骤分批调用。
- 参数错误表示尚未执行，依据错误修正后再试；已成功的修改不要因日志截断而重复执行。大输出按返回路径用 read 的 offset/limit 分段读取，避免反复获取全文。
...
# 子代理
- task 适合边界清楚、能独立研究或执行的子问题。委派时写清目标、范围、约束和期望回报；多个修改任务分配不重叠的文件范围。task 返回即代表子代理已结束并交回结果，失败时根据错误处理。
